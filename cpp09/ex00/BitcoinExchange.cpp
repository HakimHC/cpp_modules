...
void BitcoinExchange::loadDatabase() {
  ...
  if (!dataBase.is_open())
    throw std::runtime_error("fatal: cannot open data.csv");
  ...
  while (std::getline(dataBase, line)) {
    ...
    if (!validateDate(key))
      throw std::runtime_error(
          "fatal: parsing error in data.csv (invalid date)");
  ...
  while (std::getline(inFile, line)) {
    ...
    try {
      ...
      if (!(validated = validateDate(key))) {
        ...
        std::cout << value << " = "
                  << value * (this->_map.lower_bound(key))->second;
      }
...
void BitcoinExchange::solve(std::string arg) {
  try {
    this->loadDatabase();
  } catch (std::exception &e) {
    std::cerr << "fatal: error while parsing data.csv (" << e.what() << ")"
              << std::endl;
    return;
  }
  try {
    this->parseInputValues(arg);
  } catch (std::exception &e) {
